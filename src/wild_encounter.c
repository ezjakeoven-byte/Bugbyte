...
void CreateWildMon(u16 species, u8 level)
{
    ...
    // One-shot debug species override.
    if (gDebugNextWildSpecies != SPECIES_NONE)
    {
        species = gDebugNextWildSpecies;
        gDebugNextWildSpecies = SPECIES_NONE;
    }

    // Consume the one-shot shiny override.
    if (gDebugNextWildShiny)
    {
        forceShiny = TRUE;
