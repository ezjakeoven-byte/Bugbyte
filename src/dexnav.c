...
static u8 GetEncounterLevelFromMapData(u16 species, enum EncounterType environment)
{
    ...
    case ENCOUNTER_TYPE_LAND:
    {
        ...
        {
            encounterSpecies = landMonsInfo->wildPokemon[i].species;

#if RANDOMIZER_AVAILABLE == TRUE
            if (gSaveBlock3Ptr->challengeSettings.tx_Random_MapBased)
            {
                encounterSpecies = RandomizeWildEncounter(
                    encounterSpecies,
            ...
            {
                min = (min < landMonsInfo->wildPokemon[i].minLevel)
                    ? min
                    : landMonsInfo->wildPokemon[i].minLevel;

                max = (max > landMonsInfo->wildPokemon[i].maxLevel)
                    ? max
                    : landMonsInfo->wildPokemon[i].maxLevel;
            }
        }
        break;
    }
    ...
    case ENCOUNTER_TYPE_WATER:
    {
        ...
        {
            encounterSpecies = waterMonsInfo->wildPokemon[i].species;
            ...
            if (encounterSpecies == species)
            {
                min = (min < waterMonsInfo->wildPokemon[i].minLevel)
                    ? min
                    : waterMonsInfo->wildPokemon[i].minLevel;

                max = (max > waterMonsInfo->wildPokemon[i].maxLevel)
                    ? max
                    : waterMonsInfo->wildPokemon[i].maxLevel;
            }
        }
        break;
    }
    ...
    case ENCOUNTER_TYPE_HIDDEN:
    {
        ...
        {
            encounterSpecies = hiddenMonsInfo->wildPokemon[i].species;
            ...
            if (encounterSpecies == species)
            {
                min = (min < hiddenMonsInfo->wildPokemon[i].minLevel)
                    ? min
                    : hiddenMonsInfo->wildPokemon[i].minLevel;

                max = (max > hiddenMonsInfo->wildPokemon[i].maxLevel)
                    ? max
                    : hiddenMonsInfo->wildPokemon[i].maxLevel;
            }
        }

        // 0 = land hidden encounter, 1 = water hidden encounter
